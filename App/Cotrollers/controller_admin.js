...
module.exports.postAdmin = function (req, res) {
    ...
    Profile.find()
        .then(note => {
            ...
            const profil = new Profile({ _id: id, nom: nom, contact: contact, email: email, password: password });

            if (nom || contact || email || password || pwd) {
...
module.exports.updateAdmin = (req, res) => {
    ...
    if (!nom || !contact || !email || !password || !pwd) {
        ...
        (password !== pwd) ? console.log("Erreur passwords") :
            Profile.findByIdAndUpdate(req.params.id, { nom: nom, contact: contact, email: email, password: password }, (err, product) => {
                Profile.find()
